...
function RecipeForm() {
  ...
  // Fetch the existing recipe if editing
  useEffect(() => {
    if (id) {
      // Fetch the recipe details from the server if editing
      axios.get(`http://localhost:5000/recipes/${id}`, { params: { username: localStorage.getItem('username') } })
        .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    const ingredientsString = typeof recipe.ingredients === 'string'
      ? recipe.ingredients
      : recipe.ingredients.join(', '); // If it's an array, join into a string

    const requestPayload = {
      ...recipe,
      ingredients: ingredientsString.split(',').map(ingredient => ingredient.trim()), // Convert ingredients to array
      username: localStorage.getItem('username') // Send the logged-in user's username
    };
